...
type CopilotConfig struct {
	...
	Model      string
	BaseURL    string
	TokenURL   string // Token exchange URL; defaults to GitHub's Copilot token endpoint.
	MaxTokens  int
...
func NewCopilotProvider(cfg CopilotConfig) *CopilotProvider {
	...
	if cfg.TokenURL == "" {
		cfg.TokenURL = copilotTokenExchangeURL
	}
...
func (p *CopilotProvider) ensureBearerToken(ctx context.Context) error {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, p.config.TokenURL, nil)
	if err != nil {
