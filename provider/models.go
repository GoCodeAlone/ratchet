...
func listCopilotModels(ctx context.Context, apiKey, baseURL string, tokenURLOverride ...string) ([]ModelInfo, error) {
	if baseURL == "" {
	...
	tokenURL := ""
	if len(tokenURLOverride) > 0 {
		tokenURL = tokenURLOverride[0]
	}
	bearerToken, err := exchangeCopilotToken(ctx, apiKey, tokenURL)
	if err != nil {
